...
class Net(nn.Module):
    ...
    def forward(self, v, b, q, v_mask, q_mask, q_len):
        ...
        #answer = self.graphpool(v, q, v_mask, q_mask)
...
class SingleBlock(nn.Module):
    ...
    def __init__(self, num_block, v_size, q_size, output_size, num_inter_head, num_intra_head, drop=0.0):
        ...
        self.v_lin = FCNet(v_size, output_size, relu=False, drop=drop)
        self.q_lin = FCNet(q_size, output_size, relu=False, drop=drop)
...
class InterModalityUpdate(nn.Module):
    ...
    def __init__(self, v_size, q_size, output_size, num_head, drop=0.0):
        ...
        self.v_lin = FCNet(v_size, output_size * 3, relu=False, drop=drop)
        self.q_lin = FCNet(q_size, output_size * 3, relu=False, drop=drop)

        self.v_output = FCNet(output_size + v_size, output_size, relu=False, drop=drop)
        self.q_output = FCNet(output_size + q_size, output_size, relu=False, drop=drop)
    ...
    def forward(self, v, q, v_mask, q_mask):
        ...
        for i in range(self.num_head):
            ...
            q2v = (vq_slice @ qk_slice.transpose(1,2)).masked_fill(q_mask.unsqueeze(1).expand([batch_size, num_obj, max_len]) == 0, -float('inf')) / ((self.output_size // self.num_head) ** 0.5)
            v2q = (qq_slice @ vk_slice.transpose(1,2)).masked_fill(v_mask.unsqueeze(1).expand([batch_size, max_len, num_obj]) == 0, -float('inf')) / ((self.output_size // self.num_head) ** 0.5)
            # softmax attention
...
class DyIntraModalityUpdate(nn.Module):
    ...
    def __init__(self, v_size, q_size, output_size, num_head, drop=0.0):
        ...
        self.v4q_gate_lin = FCNet(v_size, output_size, relu=False, drop=drop)
        self.q4v_gate_lin = FCNet(q_size, output_size, relu=False, drop=drop)

        self.v_lin = FCNet(v_size, output_size * 3, relu=False, drop=drop)
        self.q_lin = FCNet(q_size, output_size * 3, relu=False, drop=drop)

        self.v_output = FCNet(output_size, output_size, relu=False, drop=drop)
        self.q_output = FCNet(output_size, output_size, relu=False, drop=drop)
    ...
    def forward(self, v, q, v_mask, q_mask):
        ...
        for i in range(self.num_head):
            ...
            qk_slice, qq_slice, qv_slice = qk_set[i], qq_set[i], qv_set[i]  #[batch, max_len, feat_size]
            # calculate attention
            v2v = (vq_slice @ vk_slice.transpose(1,2)).masked_fill(v_mask.unsqueeze(1).expand([batch_size, num_obj, num_obj]) == 0, -float('inf')) / ((self.output_size // self.num_head) ** 0.5)
            q2q = (qq_slice @ qk_slice.transpose(1,2)).masked_fill(q_mask.unsqueeze(1).expand([batch_size, max_len, max_len]) == 0, -float('inf')) / ((self.output_size // self.num_head) ** 0.5)
